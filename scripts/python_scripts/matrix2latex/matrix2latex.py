...
Python/MATLAB matrix to LaTeX table converter, originally by Oeystein Bjoerndal https://code.google.com/p/matrix2latex/.
...
This current version contains edits by Aleksandr M. Kazachkov. Anything written by AMK is distributed under the Eclipse Public License v2.
There is another version without AMK edits on GitHub by Horea Christian at https://github.com/TheChymera/matrix2latex. 
...
def matrix2latex(matr, filename=None, *environments, **keywords):
    ...
    **keywords
    ...
    Note that many of these options only have an effect when typesetting a table,
    ...
    # Define matrix-size
    # 
    m = len(matr) # number rows
    try:
        n = len(matr[0]) # number columns, may raise TypeError
        for row in matr:
...
#         pass
    ...
    columnIndices = range(0,n)  # Added by Alex, 2018-10-23
...
#                 newMatr.append(row)
            ...
            midruleIndex = value
            if type(midruleIndex) is not list:
                midruleIndex = [midruleIndex]
        elif key == "columnIndices":
            if (len(value) > 0):
                columnIndices = value
                if type(columnIndices) is not list:
    ...
    if formatColumn == None:
        formatColumn = list()
        for j in range(0, len(columnIndices)):
    ...
    if headerRow != None:
        for row in range(len(headerRow)): # for each header
            ...
            while i < len(columnIndices): # for each element (skipping repeating ones)
                col = columnIndices[i]
                if (col >= len(headerRow[row])):
                    break;
                j = 1
                
                # Format the output
                e = str(headerRow[row][col])
                ...
                repeating = (i+j < len(columnIndices)) and (columnIndices[i+j] < len(headerRow[row]) and (headerRow[row][col] is not '') and headerRow[row][col] == headerRow[row][columnIndices[i+j]])
                if repeating:
                    while repeating:        # figure out how long it repeats (j)
                        j += 1
                        repeating = (i+j < len(columnIndices)) and (columnIndices[i+j] < len(headerRow[row])) and (headerRow[row][col] == headerRow[row][columnIndices[i+j]])
                    f.write(r'\multicolumn{%d}{c}{%s}' % (j, formatted)) # multicol heading
                    start.append(i);end.append(i+j)
                    i += j                 # skip ahed
                else:
                    f.write('{%s}' % formatted) # normal heading
                    i += 1
                if i < len(columnIndices) and columnIndices[i] < len(headerRow[row]): # if not last element
                    f.write(' & ')
    ...
    for i in range(0, m):
        ...
        if (i in midruleIndex):
            f.write("\midrule\n"+separator*tabs)
        for tmp_j in range(0,len(columnIndices)):
            j = columnIndices[tmp_j]
            if tmp_j == 0:                  # first row
                if headerColumn != None:
                    try:
            ...
            try: # get current element
                if '%s' not in formatColumn[tmp_j]:
                    try:
            ...
            if e == None or isnan(e):#e == float('NaN'):
                ...
                fcj = '%s' if treatAsString else formatColumn[tmp_j]
                ...
                formatted = fix(formatted, table=True) # fix 1e+2
                f.write('{%s}' % formatted)
            if tmp_j != len(columnIndices)-1:                # not last column
                f.write(" & ")
            else:                       # last column
                f.write(r"\\")
                f.write("\n")
