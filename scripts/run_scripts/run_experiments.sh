#!/bin/bash
# PHA_DIR should be defined for these scripts
#
# Three arguments: 
# 1. 'sic', 'best', 'full', 'test' [required, no default]
# 2. full path to instance list, which should be located in the same directory as the instances [optional; default: ${PHA_DIR}/data/instances/test.instances]
# 3. '1' or a '0', referring to whether batch mode is used [optional; default: 0]

# Defaults
DEFAULT_INSTANCE_DIR="${PHA_DIR}/data/instances"
DEFAULT_INSTANCE_LIST="${PHA_DIR}/data/instances/test.instances"
DEFAULT_BATCH_LIST="${PHA_DIR}/data/instances/test.batch"
INSTANCE_LIST="${DEFAULT_INSTANCE_LIST}"
BATCH_MODE=0

SCRIPT_DIR="${PHA_DIR}/scripts/run_scripts"
OUT_DIR="${PHA_DIR}/results"
